...
def fetch_market_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    raise ValueError("yfinance returned no usable data after retry attempts.")
...
def resolve_session_open(intraday: pd.DataFrame, daily: pd.DataFrame) -> float:
    ...
    if not todays_bars.empty:
        return float(todays_bars["Open"].iloc[0]) # FIXED: Explicit array bracket tracking index [0] included properly
...
def send_telegram_alert(text: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            print("Toon Telegram target confirmed — alert sent!")
            return True
        else:
